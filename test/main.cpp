...
public:
    ...
    Record(string playerName)
    {
        ...
        if(inFile)
        {
            cout << "Player record found. Here is " + playerName + "'s record:" << endl;
            inFile >> maxCards >> maxPoints >> maxWinStreak;
            this->print();
        }
        else
        {
            cout << "Player record does not exist. Establishing new record:" << endl;
            maxCards = 0; maxPoints = 0; maxWinStreak = 0;
            ofstream recordFile(filePath);
            if(!recordFile)
                cout << "ERROR! Failed to established record!" << endl;
            else
            {
                recordFile << maxCards << " "<< maxPoints << " " << maxWinStreak;
                this->print();
            }
            recordFile.close();
        }
    ...
    // 更新玩家檔案
    void updateRecord(int cardCnt, int pointCnt, int winStreak)
    {
        if(cardCnt > maxCards || pointCnt > maxPoints
